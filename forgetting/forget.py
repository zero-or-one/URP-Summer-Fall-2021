...
def fine_tune(model, loss, optimizer, scheduler, epochs, device, dataset, lossfn, disable_bn, train_loader,
    weight_decay=0.0, lr=0.001, momentum=0.9): # and retrain
    start_time = time.time()
    train(model, loss, optimizer, scheduler, epochs, device, dataset, lossfn, disable_bn, train_loader, None,
    weight_decay, lr, momentum)
    print("Process time is:", time.time() - start_time)

def rapid_retraining(): # deltagrad
    pass

def hiding(model, rem_class):
    pass

def neg_gradient(model, loss, optimizer, scheduler, epochs, device, dataset, lossfn, train_loader,
